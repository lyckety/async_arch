...
type EnvSettings struct {
	...
	GroupID    string `env:"GROUP_ID" env-default:"test-consumer" env-description:"gpoup id for consumer. Must set GROUP_ID or PARTITION"`                                    //nolint:lll
	Partition  int    `env:"PARTITION" env-default:"0" env-description:"listen partition. Must set GROUP_ID or PARTITION"`                                                    //nolint:lll
	ReaderType string `env:"READER_TYPE" env-default:"fetch" env-description:"type read messages: \"fetch\" or \"read\" "`                                                    //nolint:lll
...
func New() *Config {
	...
	if err != nil {
		logger.Panic("getting help string of env settings failed: ", err)
	}

	if err := cleanenv.ReadEnv(cfg.env); err != nil {
		logger.Panicf("%s: %s", ErrReadEnvSettings, err)
	}
